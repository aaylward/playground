...
void light_loop(int delay_millis, int step, Direction direction) {
    ...
    while (pixeln <= CIRCUIT_PLAYGROUND_NUMBER_OF_PIXELS) {
      actual_pixel = direction == clockwise ? CIRCUIT_PLAYGROUND_NUMBER_OF_PIXELS - pixeln : pixeln;
      actual_pixel = (actual_pixel * step) % CIRCUIT_PLAYGROUND_NUMBER_OF_PIXELS;
